...
anon `auth/v1/signup` returns NO session (confirmation_sent_at set) → can't finish. Use the service_role key + Admin API instead: `POST ${SUPABASE_URL}/auth/v1/admin/users` with `{email,password,email_confirm:true,user_metadata:{...}}`, then password-grant login to get a token, then `POST <api>/api/auth/sync` (ProfileInput body) to create the `profiles` row. NOTE: `/auth/sync` uses the JWT `sub` for the id, ignoring `body.id`. Bash gotcha: `UID` is readonly — never assign to it.
