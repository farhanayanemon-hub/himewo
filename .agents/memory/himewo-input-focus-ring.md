...
GOTCHA: `:focus-visible` **matches text inputs on plain mouse-click/tap** (browsers always show focus indication for text entry), NOT keyboard-only. So you canNOT use `:focus-visible` to "only show a ring for keyboard users" on inputs — the ring shows on tap too. To fully remove the focus border/ring on inputs (Apple-style, per user request), set `outline:none !important;box-shadow:none !important` on BOTH `:focus` and `:focus-visible` for `input,textarea`. Also removed matching purple focus styling on web (shadcn `ui/input.tsx`/`ui/textarea.tsx` + hardcoded auth/signup/recovery inputClass used `focus-visible:ring-*`/`border-ring` where `--ring` is purple) and native RN inputs (login `LabeledInput` + `AccountRecovery` used `borderColor: isFocused ? c.primary : c.border`). Keep the purple on SELECTION chips/tabs (marketplace/pages/settings/tabbar) — that's intended selected-state, not input focus.
