      ...
      <div className="min-w-0 flex-1">
        ...
        {replies && replies.length > 0 && showReplies && (
          <div className="mt-3 space-y-3 border-l-2 border-border/60 pl-3">
            {replies.map((r) => (
              <CommentItem key={r.id} comment={r} postId={postId} onReply={onReply} onChanged={onChanged} />
            ))}
            <button
...
/**
 * Full comments section for a post: composer (with mentions, GIF, emoji,
 * reply targeting) plus the threaded comment list. Shared by the post
 * detail page and the Facebook-style photo viewer panel.
 */
export function PostComments({
  ...
  /** Extra invalidation hook — e.g. the photo viewer refreshes feed/profile queries so comment counts stay in sync. */
  onChanged?: () => void;
}) {
