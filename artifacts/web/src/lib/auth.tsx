...
const OTP_API_BASE = (() => {
  const raw = import.meta.env.DEV ? undefined : ((import.meta.env.VITE_API_URL as string | undefined) || "https://api-server-production-be15.up.railway.app");
  if (!raw) return "https://api-server-production-be15.up.railway.app";
...
async function assertOtpAllowed(event: string): Promise<void> {
  ...
  try {
    const res = await fetch(`${OTP_API_BASE}/api/auth/otp-config`);
