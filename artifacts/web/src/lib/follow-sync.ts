 ...
 * reflect the new follow state (`isFollowing`), and triggers invalidations so
 * server state syncs seamlessly.
 */
export function syncUserFollowState(
  ...
  // 2. Update user posts queries (e.g. ['/api/users', id, 'posts'])
  queryClient.setQueriesData<Post[]>(
    {
      predicate: (query) => {
        ...
        return (
          ...
          key.some(
            (part) =>
              typeof part === "string" &&
              (part.includes(`/api/users/${userId}/posts`) || part === `/api/users/${userId}/posts`),
          )
        );
      },
    },
  ...
  queryClient.setQueriesData<Reel[]>(
    {
      predicate: (query) => {
        const key = query.queryKey;
        return Array.isArray(key) && key[0] === "user-reels";
      },
    },
    (old) => {
      if (!old || !Array.isArray(old)) return old;
